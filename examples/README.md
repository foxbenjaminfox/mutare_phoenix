...
`mutare_phoenix` families. Run it from the repo root (compile the package first so its
mutators are on the code path):
...
| --- | --- | --- |
| [`demo`](demo/) | Plug + controller | A forgotten `halt` (`:plug_halt`) and an unasserted status (`:http_status`) — survivors in both families, plus the kills that prove the families catch what *is* asserted. |
