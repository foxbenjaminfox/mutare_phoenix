defmodule Mutare.Phoenix.PubSub do
  @moduledoc """
  `:pubsub` — removes a `Phoenix.PubSub` call, collapsing it to the `:ok` it returns on its
  happy path. Three kinds, each a variant label:
  ...
  process reacted — the LiveView that subscribes in `mount/3` and re-renders on
  ...
  channel-level `Phoenix.Channel.broadcast/3` family resolves to a different module and is
  `:channel_message`.

  Only the real arities fire, so a name-matched call of any other arity (reachable only by an
