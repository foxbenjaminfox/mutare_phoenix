defmodule Mutare.Phoenix.Response do
  ...
  Configuring a family means listing it yourself, so expand `Mutare.Phoenix.all/0` into its
  members and replace the `Response` entry:
  ...
  An unconfigured instance — `Mutare.Phoenix.Response`, or the one in `Mutare.Phoenix.all/0`
  ...
  # model. Skipping the DSL keeps core from wasting mutant ids on it. Plug
  ...
  # The override map from a `{Response, swaps: %{…}}` configuration, keeping only structurally
  ...
  # the same name-only stance the package takes toward `Plug.Conn` / `Phoenix.Controller`
