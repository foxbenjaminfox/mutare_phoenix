defmodule Mutare.Phoenix.PlugTest do
  @moduledoc """
  ...
  alias Mutare.Phoenix.Plug, as: PlugHalt

  defp halt_diffs(source), do: diffs_for(source, [PlugHalt], :plug_halt)

  # Wrap a plug body in a module that imports `Plug.Conn`, the `use MyAppWeb, :plug` form.
  ...
  describe "pure-AST node path (mutations/3)" do
    test "qualified halt removes to its first argument" do
      assert node_mutations("Plug.Conn.halt(conn)", PlugHalt) == ["conn"]
    end

    test "a piped stage node yields the identity no-op" do
      assert node_mutations("Plug.Conn.halt()", PlugHalt, :piped) == [
  ...
  test "every embedded mutant compiles" do
    ...
    assert_metamutant_compiles(source, [PlugHalt])
  end
end
