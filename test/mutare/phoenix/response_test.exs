defmodule Mutare.Phoenix.ResponseTest do
  ...
  alias Mutare.Phoenix.Response

  defp status_diffs(source), do: diffs_for(source, [Response], :http_status)

  # The same, for a `{Response, opts}` configured instance (see "configurable :swaps table").
  defp configured_diffs(source, opts), do: diffs_for(source, [{Response, opts}], :http_status)
  ...
  describe "configurable :swaps table" do
    ...
    test "configured siblings are trusted as-is, not checked against real statuses" do
      # The package matches Plug.Conn / Phoenix.Controller by name and never calls into them;
  ...
  describe "superseding the crashing atom leaves (Overlap)" do
    test "with :atom also enabled, only the valid swaps remain — no :mutare leaf" do
      source = "defmodule C do\n  def show(c), do: Plug.Conn.put_status(c, :ok)\nend\n"
      pairs = diffs_for(source, [Response, :atom], :http_status)
      atom_pairs = diffs_for(source, [Response, :atom], :atom)
    ...
    test "the ConventionAtom :ok -> :error leaf is also superseded (it crashes as a status)" do
      ...
      source = "defmodule C do\n  def show(c), do: Plug.Conn.put_status(c, :ok)\nend\n"
      convention_pairs = diffs_for(source, [Response, :convention], :convention)
  ...
  describe "pure-AST node path (mutations/3)" do
    test "qualified put_status swaps the atom status" do
      assert node_mutations("Plug.Conn.put_status(conn, :ok)", Response) == [
    ...
    test "qualified send_resp swaps the atom status" do
      assert node_mutations("Plug.Conn.send_resp(conn, :not_found, \"\")", Response) == [
    ...
    test "a bare (non-block-wrapped) atom status swaps via the AST.literal fallback" do
      ...
      assert bare_node_mutations(node, Response) == [
               "Plug.Conn.put_status(conn, :created)",
               "Plug.Conn.put_status(conn, :no_content)"
             ]
    end
  ...
  test "every embedded mutant compiles" do
    ...
    assert_metamutant_compiles(source, [Response])
  end
end
